...
from models.resnet_cifar_gnws import InsResNet50_cifar_gnws
from models.LinearModel import LinearClassifierResNet


from dataset import CIFAR10Instance
...
def parse_option():
    ...
    parser.add_argument('--lr_cycle', type=float, default=1, help='Cosine decay with warm restart cycle interval')
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='imagenet100', choices=['imagenet100', 'imagenet', 'cifar10'])
    ...
    opt.model_name = opt.test_path.split('/')[-2] + '_layer{}'.format(opt.layer) + "_lr{}".format(opt.learning_rate) + '_cosine'
...
def main():
    ...
    elif args.aug == 'simple' and args.dataset == 'cifar10':
        train_transform = transforms.Compose([
    ...
    if 'imagenet' in args.dataset:
        ...
        val_dataset = CIFAR10Instance(root='./data', train=False, download=True, transform=test_transform)
        val_loader = torch.utils.data.DataLoader(val_dataset,
                                                 batch_size=200, shuffle=False, num_workers=args.num_workers)
